...
pub fn by_name(name: &str, integration_branch: &str) -> Option<Box<dyn RoutingStrategy>> {
    match name {
        "trunk" => Some(Box::new(Trunk::new(integration_branch))),
        "phase_stacking" => Some(Box::new(PhaseStacking)),
        _ => None,
    }
}
